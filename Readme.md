...
- Включаем алерты User settings → Notifications → Add → Выбрать созданный тип → В поле Send to указать ID канала\группы\чата
  (либо в админке, либо идем в "О программе" -> "Начать сбор диагностических данных" -> Тыкаемся в наш чат -> Останавливаем сбор-> Ищем в логах groupChatId и сопоставляем с именем группы.)
